...
data_table <- languages %>% 
  ...
  arrange(Name) %>% 
  xtable(
    caption="Extended Data table with Glottocode, number of speakers, number of phones, and source for each dataset.",
    label="table:data"
    )
...
data %>% 
  mutate(initial=as.factor(ifelse(
...
map_doreco <- ggplot() +
  ...
  geom_jitter(
    ...
    height=3, width=3, size=6, shape=21, alpha=0.8
    ) +
