SELECT
    ...
    CASE
        WHEN phone.cldf_id in (select cldf_id FROM utterance_final) THEN 1 ELSE 0
        END utt_final, -- whether or not the phone is in utterance initial position
	CASE
    ...
    CASE
		WHEN sound.cldf_cltsReference LIKE '%long%' THEN 'long'
        WHEN sound.cldf_cltsReference LIKE '%nasalized%' THEN 'nasal' ELSE 'vowel'
        END sound_class,
...
WHERE
    ...
    sound.cldf_cltsReference NOT LIKE '%_consonant' AND
